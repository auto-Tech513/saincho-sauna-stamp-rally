...
- `sw.js` は `saincho-v10` として、HTML/CSS/JS/データ/manifest/主要アイコン/ヒーロー画像をキャッシュします。
...
- 初期候補は `data.js` に分離し、SAUNACHELIN公表ランキング由来の候補、SaunaTime都道府県ページから取得した県別スターター候補、愛知・岐阜・三重・静岡の指定候補を合わせて469件に拡張しました。
- 各都道府県に6件以上の候補が入るため、候補が空の県はありません。
- 愛知県は54件、岐阜県は43件、三重県は52件、静岡県は53件です。ID重複と県内名称重複は検証で0件にしています。
...
- 施設ごとのメモは保存・取り消し・X共有ができます。メモは `facilityMemos` として施設IDに紐づけて保存します。
