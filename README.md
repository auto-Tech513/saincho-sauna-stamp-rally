...
- `sw.js` は `saincho-v22` として、HTML/CSS/JS/データ/manifest/主要アイコン/ヒーロー画像/OG画像/称号イラスト/SEOファイルをキャッシュします。
...
- 初期候補は `data.js` に分離し、SAUNACHELIN公表ランキング由来の候補、SaunaTime都道府県ページから取得した県別スターター候補、指定候補、県別30件補強候補を合わせて1589件に拡張しました。
- 各都道府県に30件以上の候補が入るため、候補が空の県はありません。
- 2026年7月2日の検証で、47都道府県すべて30件以上、県内名称重複0件、不自然な内部タグの表示0件、sourceUrl未設定0件です。
