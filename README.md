...
ローカルサーバーまたはCloudflare Pagesで配信してください。`/gear/` や `/guide/` の直リンク対応のため、ファイル直開きではなくHTTP配信が前提です。記録はブラウザの `localStorage` に保存され、記録タブから JSON で書き出し・読み込みできます。
...
- `sw.js` は `saincho-v23` として、HTML/CSS/JS/データ/manifest/主要アイコン/ヒーロー画像/OG画像/称号イラスト/SEOファイルをキャッシュします。
- `/gear/`、`/guide/`、`/ranking/`、`/passport/` は Cloudflare Pages の `_redirects` で SPA として開けます。各画面の title、description、canonical、OG URL は JavaScript で更新します。
- `llms.txt` を追加し、AI検索・回答エンジン向けにサ印帳の概要と主要URLを明示しています。
- 記録画面と設定メニューに共有導線を追加し、Web Share API、LINE共有、URLコピーで紹介しやすい状態にしています。
- Explore の「次の一湯」カードから施設特徴に合う Gear へつなぎ、サウナ入門にはFAQと関連Gearを表示します。
...
- 初期候補は `data.js` に分離し、SAUNACHELIN公表ランキング由来の候補、SaunaTime都道府県ページから取得した県別スターター候補、指定候補、サウナイキタイ県別ページから確認した県別50件補強候補を合わせて2376件に拡張しました。
- 各都道府県に50件以上の候補が入るため、候補が空の県はありません。
- 2026年7月2日の検証で、47都道府県すべて50件以上、県内名称重複0件、ID重複0件、不自然な表示タグ0件、sourceUrl未設定0件です。
- サウナイキタイ由来の追加候補は、各施設の直接ページを `sourceUrl` に設定しています。施設詳細ページのHP欄で確認できたものだけ `officialUrl` を併記し、未確認の公式URLは推測で入れていません。
